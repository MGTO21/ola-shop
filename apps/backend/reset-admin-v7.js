...
async function resetAdminV7() {
    ...
    const p = 1;
    const keyLen = 32;
    const maxmem = 64 * 1024 * 1024; // Increase to 64MB to avoid memory limit error

    crypto.scrypt(password, salt, keyLen, { N, r, p, maxmem }, async (err, derivedKey) => {
        if (err) {
